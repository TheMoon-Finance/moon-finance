...
class Want extends Component {
  ...
  render() {
    ...
    if (bestPrice && bestPrice.price > 0 && sendAmount && sendAmount > 0) {
      amount = (parseFloat(bestPrice.price) * parseFloat(sendAmount)).toFixed(
        4
      );
    }
    ...
    return (
      <div className={classes.root}>
        <div className={classes.inputCard}>
          ...
          {sendAsset &&
            sendAsset.symbol === "ETH" &&
            this.renderAsset("DAI", amount)}
            ...
            <InputAdornment position="start" className={classes.inputAdornment}>
              <div className={classes.assetSelectIcon}>
                <img
                  alt=""
                  src={require("../../assets/" +
                    (["crvV1", "crvV2", "crvV3", "crvV3", "crv"].includes(id)
                      ? "CRV"
                      : id) +
                    "-logo.png")}
                  height="30px"
          ...
          ? options
              .filter((option) => {
                ...
                if (sendAsset && ["crvV3"].includes(sendAsset.id)) {
                  return (
                    ["crvV1", "crvV2", "crvV3", "ETHv1", "BUSDv3"].includes(
                      option.id
                    ) === false
                  );
                }
        ...
        <React.Fragment>
          <div className={classes.assetSelectIcon}>
            <img
              alt=""
              src={require("../../assets/" +
                (["crvV1", "crvV2", "crvV3", "crvV4"].includes(option.id)
                  ? "CRV"
                  : option.symbol) +
                "-logo.png")}
              height="30px"
